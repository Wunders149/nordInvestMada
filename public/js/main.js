...
async function loadTeam() {
  try {
    ...
    const teamGroup = (members, category) => {
      ...
      return `<div class="team-group">
        ...
        <div class="team-grid" aria-label="${category.label}">${members.map(memberCard).join('')}</div>
      </div>`;
    };
    grid.innerHTML = categories.map(category => teamGroup(team.filter(m => (m.group_type || m.groupType || 'office') === category.id), category)).join('') || '<p class="team-empty">Notre équipe sera bientôt présentée ici.</p>';
    loadImageSlots();
